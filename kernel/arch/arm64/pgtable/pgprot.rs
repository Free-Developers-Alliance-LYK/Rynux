...
use crate::{arch::arm64::sysregs::MairAttrIdx, bitflags::bitflags, klib::bits::genmask64};
...
impl PtePgProt {
    ...
    pub const PTE_TYPE_PAGE: Self =
        Self::from_bits_truncate(Self::PTE_VALID.bits() | Self::PTE_NON_BLOCK.bits());
    ...
    pub const PROT_DEFAULT: Self = Self::from_bits_truncate(
        Self::PTE_TYPE_PAGE.bits()
            | Self::pte_maybe_shared().bits()
            | Self::pte_maybe_ng().bits()
            | Self::PTE_AF.bits(),
    );
    ...
    pub const PAGE_KERNEL_RO: Self = Self::from_bits_truncate(
        Self::PROT_NORMAL.bits() & !(Self::PTE_WRITE.bits()) | Self::PTE_RDONLY.bits(),
    );

    /// Page kernel read only execute
    pub const PAGE_KERNEL_ROX: Self = Self::from_bits_truncate(
        Self::PROT_NORMAL.bits() & !(Self::PTE_WRITE.bits() | Self::PTE_PXN.bits())
            | Self::PTE_RDONLY.bits(),
    );
}
