...
pub static GLOBAL_FDT: OnceCell<LinuxFdtWrapper<'static>> = OnceCell::new();

impl<'a> LinuxFdtWrapper<'a> {
    pub(crate) fn setup(fdt_va: VirtAddr) {
        // SAFETY: fdt from_ptr would check header and magic number
        GLOBAL_FDT.set(unsafe {
            LinuxFdtWrapper {
                fdt: LinuxFdt::from_ptr(fdt_va.as_usize() as *const u8).expect("Invalid fdt"),
            }
        });
    }
}
