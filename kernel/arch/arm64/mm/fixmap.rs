...
use crate::{
    ...
    macros::{section_bss_page_aligned, page_aligned, section_init_text},
...
pub enum FixMapType {
    ...
    FdtEnd = 1,
    /// Fdt
    Fdt = Self::FdtEnd as isize + div_round_up(InitIdmap::MAX_FDT_SIZE, PageConfig::PAGE_SIZE) as isize + 1,
    /// Early con mem base.
    EarlyConMemBase,
...
impl From<VirtAddr> for FixMapType {
    fn from(addr: VirtAddr) -> Self {
        let idx = (Arm64VaLayout::FIXMAP_TOP - (addr.as_usize() & PageConfig::PAGE_MASK)) >> PageConfig::PAGE_SHIFT;
        FixMapType::from(idx)
    }
}
...
const PTE_TABLES_CNT: usize =  Arm64PgtableConfig::spann_nr_entries(Arm64VaLayout::FIXMAP_START, Arm64VaLayout::FIXMAP_TOP, PmdTable::SHIFT);

#[section_bss_page_aligned]
static mut FIXMAP_PTE_TABLES: [FixMapPteTable; PTE_TABLES_CNT] =
    [FixMapPteTable { _ptes: [PteEntry::new(0); PteTable::PTRS] }; PTE_TABLES_CNT];
...
impl FixMapPteTable {
    ...
    unsafe fn mut_ptr(addr: VirtAddr) -> *mut PteEntry {
        unsafe {
            ...
            core::ptr::addr_of_mut!((FIXMAP_PTE_TABLES[idx])._ptes[0])
                as *mut PteEntry
        }
    }
}
...
impl FixMap {
    ...
    pub const FIXMAP_PERMANENT_SIZE: usize = (FixMapType::EndPermanentFixMap as usize)  <<  PageConfig::PAGE_SHIFT;
    /// Fixmap virtual address space size
    ...
    fn early_fixmap_init_pmd(pmd_tbl: &mut PmdTable,  addr: VirtAddr, end: VirtAddr) {
        ...
        while cur < end {
            ...
            if pmd_entry.is_none() {
                ...
                pmd_entry.write(pte_tbl.to_phys().as_usize() as u64 
        ...
        match &mut pud_tbl.downgrade_to_pmd_table() {
            ...
            None => {
                ...
                if pud_entry.is_none() {
                    pud_entry.write(pmd_tbl.to_phys().as_usize() as u64 
        ...
        match &mut pgdir_tbl.downgrade_to_pud_table() {
            ...
            None => {
                ...
                if pgdir_entry.is_none() {
                    pgdir_entry.write(pud_tbl.to_phys().as_usize() as u64 
    ...
    fn set_fixmap(idx: FixMapType, phys: PhysAddr, prot: PtePgProt, clear: bool) -> VirtAddr {
        ...
        if clear {
            ...
            pte_entry.write(
                PteEntry::from_phys(phys_base).value()
                | prot.bits()
            );
        }
        // return virt addr
    ...
    pub fn clear_pud_map() {
        Self::set_fixmap(FixMapType::PudMap, PhysAddr::from(0), PtePgProt::empty(), true);
    }
    ...
    pub fn clear_pmd_map() {
        Self::set_fixmap(FixMapType::PmdMap, PhysAddr::from(0), PtePgProt::empty(), true);
    }
    ...
    pub fn clear_pte_map() {
        Self::set_fixmap(FixMapType::PteMap, PhysAddr::from(0), PtePgProt::empty(), true);
    }
